...
Tests Appendix E corrected formula:
  ...
  - Comfort  (50%): mean ED3 proximity margin vs 1,000 m
  (Separation term removed — spreading schools apart pushes them away from shelters.)
...
#       cap=10, comfort=1000→10
#       sub = round(0.50*10 + 0.50*10) = round(10) = 10
sh = [_shelter(300, 300)]
sc1 = [_school(300, 300)]
...
#       cap=10, comfort=0→0
#       sub = round(0.50*10 + 0.50*0) = round(5.0) = 5
...
check("Shelter 1000 m from school -> comfort=0, score = 5", pts == 5, f"got {pts} -- {expl}")

# ── 5. 2 schools poorly separated (50 m apart), 2 req
#       Shelters co-located with schools → comfort=1000→10.
#       cap=10, comfort=10
#       sub = round(0.50*10 + 0.50*10) = round(10) = 10
#       Separation is no longer scored.
...
check("2 schools 50 m apart, shelters co-located -> score = 10", pts_close == 10, f"got {pts_close}")

# ── 6. 2 schools well separated (400 m apart), 2 req — same score as closely-spaced
#       Shelters co-located with schools → comfort=1000→10.
#       cap=10, comfort=10
#       sub = round(0.50*10 + 0.50*10) = 10
#       Separation not scored — both arrangements earn the same score.
...
check("Separation not rewarded — same score as closely-spaced", pts_far == pts_close,
      f"far={pts_far} close={pts_close}")

# ── 7. Only 1 school placed when 2 required
#       cap = round(1/2*10) = 5; comfort=1000→10
#       sub = round(0.50*5 + 0.50*10) = round(7.5) = 8 (banker's)
